...
#create renv project environment #renv::init()
...
Pipeline depends on the following packages: 'tidyverse', 'DESeq2', 'DT'
and 'ggrepel'. NOTE - in order to run Rmarkdown each package needs to be
already installed in you R for pipeline to work.
...
In this stage, we prepare the count matrix and metadata files. The count
matrix contains raw gene expression counts across different samples,
while the metadata file includes essential information such as sample
identifiers, conditions, and covariates. Properly formatted input files
are crucial for the subsequent analysis steps.
...
Normalization is performed to account for systematic biases and
technical variations across samples. We utilize methods like the
DESeq2's normalization procedure, which applies a size factor to each
sample based on the geometric mean of counts, ensuring that the data is
...
In this phase, we identify differential expressed genes (DEGs) between
the specified conditions. Using the DESeq2 package, we fit a generalized
linear model to the count data and perform statistical tests to estimate
the log2 fold change and adjusted p-values for each gene. This step
highlights genes that exhibit significant changes in expression related
to the conditions under study.

NOTE: Following steps are just for the purpose of this exercise. For
more detailed examples and more comprehansive overview of DEA refer to
[DESeq2
vignette](https://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html#indfilt).
...
The results table presents the differential expressed genes (DEGs)
identified through our analysis. Each row in the table corresponds to a
specific gene, providing key metrics such as the gene name, base mean
expression, log2 fold change, p-value and adjusted p-value.
...
Results visualization is essential for interpreting the findings. We
generate various plots, including volcano plots to visualize the
relationship between fold change and significance, and heatmaps for
visualizing the expression patterns of DEGs. These visualizations help
in understanding the biological significance of the results and in
